...
propagates what the datasheets do state into an *interval* of admissible
profiles, and reports the ΔE2000 each legacy model scores across that
interval.
...
def evaluate(cmf) -> dict:
    ...
    for part, channels in PARTS.items():
        ...
        lum_options = {
            name: sorted({channels[name][1][0], channels[name][1][1]})
            for name in CHANNELS
        }
        nominal_xy = [
        ...
        for model, correction in MODELS.items():
            ...
            per_model[model] = {
                ...
                "within_budget_everywhere": max(medians) <= BUDGET_MEDIAN
                and max(p95s) <= BUDGET_P95,
                "within_budget_nowhere": min(medians) > BUDGET_MEDIAN
                or min(p95s) > BUDGET_P95,
            }
        rows.append(
            {
                "part": part,
                "admissible_profiles": len(xy_options["red"]) ** 3
    ...
    return {
        ...
        "evaluation": {
            ...
            "sampling": "dominant wavelength {min, mid, max} x FWHM {low, mid, high} per channel; intensity {min, max} per channel",
        },
...
def render_report(data: dict) -> str:
    ...
    for row in data["rows"]:
        for model, r in row["models"].items():
            if r["within_budget_everywhere"]:
                verdict = "within budget for every admissible profile"
            elif r["within_budget_nowhere"]:
                verdict = "outside budget for every admissible profile"
            else:
                verdict = "undetermined: datasheet ranges straddle the budget"
            lines.append(
    ...
    outside = sum(1 for _, _, r in cells if r["within_budget_nowhere"])
    inside = sum(1 for _, _, r in cells if r["within_budget_everywhere"])
    lines += [
        "",
        "## What this establishes",
        "",
        f"- **Datasheet ranges cannot pin accuracy to the A1 budget.** In {wide} of {len(cells)} part/model",
        f"  cells the admissible median ΔE2000 alone spans more than the whole {BUDGET_MEDIAN:g} median budget,",
        ...
        f"- **Legacy models under A1:** {outside} of {len(cells)} cells are outside budget for every admissible",
        f"  profile, {inside} within budget for every one, and the rest undetermined.",
